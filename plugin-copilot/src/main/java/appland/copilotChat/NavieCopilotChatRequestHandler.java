...
public class NavieCopilotChatRequestHandler extends HttpRequestHandler {
    ...
    public boolean process(@NotNull QueryStringDecoder queryStringDecoder,
        ...
        var requestName = requestPath.substring(BASE_PATH.length());

        if ("/chat/completions".equals(requestName)) {
            // verify the authorization header, which must contain our random ide session id
            var expectedHeaderValue = "Bearer " + GitHubCopilotService.RandomIdeSessionId;
            if (!StringUtil.equals(fullHttpRequest.headers().get(HttpHeaderNames.AUTHORIZATION), expectedHeaderValue)) {
                return false;
            }
    ...
    private void handleChatCompletions(@NotNull FullHttpRequest fullHttpRequest,
                                       @NotNull ChannelHandlerContext channelHandlerContext) {
        var chatSession = cachedCopilotChat();
        if (chatSession == null) {
        ...
        var openAIRequest = GitHubCopilotService.gson.fromJson(requestBody, OpenAIChatCompletionsRequest.class);

        var copilotModel = getCopilotModel(openAIRequest.model());
    ...
    private void sendCompleteChatResponse(@NotNull CopilotChatSession chatSession,
        ...
        var listener = new CopilotToOpenAICompletionsConverter() {
            ...
            public void end() {
                ...
                var responseJson = GitHubCopilotService.gson.toJson(response);
                sendData(responseJson.getBytes(StandardCharsets.UTF_8),
                        "_.json",
                        fullHttpRequest,
    ...
    private void sendStreamingChatResponse(@NotNull CopilotChatSession chatSession,
        ...
        var converter = new CopilotToOpenAICompletionsConverter() {
            @Override
            protected void onNewChunk(@NotNull OpenAIChatResponseChunk chunk) {
                var json = GitHubCopilotService.gson.toJson(chunk);
    ...
    private volatile CopilotChatSession _cachedChat;

    private @Nullable CopilotChatSession cachedCopilotChat() {
        if (_cachedChat == null) {
            _cachedChat = GitHubCopilotService.getInstance().createChat();
        }
        return _cachedChat;
    }

    private List<CopilotModelDefinition> _cachedModels;

    private List<CopilotModelDefinition> cachedCopilotModels() throws IOException {
    ...
    private @Nullable CopilotModelDefinition getCopilotModel(@NotNull String model) {
        try {
            ...
            return models.stream()
                    ...
                    .findFirst()
                    .orElseGet(models.stream().filter(m -> "gpt-4o".equals(m.id())).findFirst()::get);
        } catch (Exception e) {
            LOG.warn("Failed to load GitHub Copilot models", e);
            return null;
        }
    }
}
