    ...
    protected void handleMessage(@NotNull String messageId, @Nullable JsonObject message) {
        ...
        switch (messageId) {
            case "choose-files-to-pin":
                ...
                var link = message != null ? message.getAsJsonPrimitive("link") : null;
                if (link != null) {
                    var path = StringUtil.trimStart(link.toString(), "file://");

                }
                break;
            case "click-link": {
                var pathSpec = message != null ? message.getAsJsonPrimitive("path") : null;
                if (pathSpec != null) {
                    handleClickLink(StringUtil.trimStart(pathSpec.toString(), "file://"));
                }
                break;
            }
            case "open-appmap": {
